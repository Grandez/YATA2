...
# Columns:
#   type: aplica types
#   name: titulo de la columna, por defecto Title Case
# 
# Options
# types es una secuencia de caracteres: 
#       a - amount
#       b - button
#       i - entero
#       l - label
...
# color Aplica a columns y a rows
#   d - Fecha comparada contra hoy
#################################
yuiTable           = function(id, width = "auto", height = "auto", inline = FALSE) {
  reactable::reactableOutput(id, width, height, inline)
}
createTableDef = function(df, ...) {
    # Crea un objeto YATATable
    # Incluye meta informacion acerca del data.frame
    args = args2list(...)
    dtbl = list()
    dcol = lapply(1:ncol(df), function(x) list(idx=x, name=colnames(df)[x]))
    names(dcol) = colnames(df)
    if (!is.null(args$types)) {
       for (idx in 1:nchar(args$types)) dcol[[idx]]$type = substr(args$types, idx, idx)
    }
    if (!is.null(args$titles)) {
       for (idx in 1:nchar(args$titles)) dcol[[idx]]$title = substr(args$titles, idx, idx)
    }
    if (!is.null(args$colors)) {
       for (idx in 1:nchar(args$colors)) dcol[[idx]]$color = substr(args$colors, idx, idx)
    }
    if (!is.null(args$target)) dtbl$target = args$target
    if (!is.null(args$event))  dtbl$event  = args$event

    lst = list(table=dtbl, columns=dcol)
    class(lst) = c(class(lst), "YATATable")
    lst
  
}
updTable          = function(data, def) { renderReactable({ .updTable3(data, NULL)       })}

.updTable3 = function(data, selection) {
   click = NULL
   
   df = data$df
   df = .adjustValues (data)
   fmt = .formatData  (df,  data)
   lst = .addButtons  (df, fmt, data)
browser()
   reactable::reactable(lst$df, striped = TRUE, compact=FALSE
                                  , pagination=FALSE
                                  , selection = selection
                                  , wrap = FALSE
                                  , highlight = TRUE
                                  , onClick = JS(.makeScript2(data$table))
                                  , columns = lst$fmt
    )

}

yuiTable2          = function(id)   { reactableOutput(id) }


updTableSingle2    = function(data) { renderReactable({ .updTable2(data, "single")   })}
updTableMultiple2  = function(data) { renderReactable({ .updTable2(data, "multiple") })}
updTbl2            = function(data) {                   .updTable2(data, "multiple") }
updTableSelection2 = function(table, sel) { updateReactable(table, selected = sel) }

   # La idea es:
   # Si le damos un tipo, ej: session, conocemos los campos y sus atributos
   # Es algo como una macro para tablas conocidas
   # Crea la estructura en base a las columnas y datos
   # Podemos crear el conjunto makeTablexxx para ahorrar el tipo
 makeYATATable = function(df, ...) {
    # Crea un objeto YATATable
    # Incluye meta informacion acerca del data.frame
    args = args2list(...)
    dtbl = list()
    ...
    lst = list(df=df, table=dtbl, columns=dcol)
    class(lst) = c(class(lst), "YATATable")
    lst
}
...
.addButtons = function(df, fmt, data) {
    if (is.null(data$buttons) || length(data$buttons) == 0) return (list(df=df, fmt=fmt, btns=FALSE))

    buttons = lapply(data$buttons, function(btn) {
                         do.call(colDef, list( name = "", sortable = FALSE
                                              ,width = 48
                                              ,style=list(`text-align` = "center")
                                              ,cell = function() btn))
    })
    if (is.null(names(data$buttons))) {
         btnNames = c(paste0("Button", seq(1, length(info$buttons))))
    } else {
         btnNames = names(data$buttons)
    }
    ...
    if (length(fmt) >  0) fmt = list.merge(fmt, buttons)
    if (length(fmt) == 0) fmt = buttons
    list(df=df,fmt=fmt, btns=TRUE)
}
...
.adjustValues = function (data) {
   ...
   for (idx in 1:ncol(df)) {
      type = data$columns[[idx]]$type
      if (is.null(type)) next
      if (type == "v") {  # Value
         df[,idx] = if (df[,idx] > 99) df[,idx] = round(df[,idx],2)
                    else if (df[,idx] > 1) df[,idx] = round(df[,idx],3)
                    else df[,idx] = round(df[,idx],6)
      }
      if (type == "P") df[,idx] = df[,idx] / 100
      if (type == "d") df[,idx] = as.Date(df[,idx])
      
   }
   df
}

.formatColumn = function(idx, data) {
   colF = list(locales = "es-ES")
   type = data$columns[[idx]]$type
   if (is.null(type)) return (NULL)
   if (type %in% c("v","a", "p","P")) colF$separators = TRUE
   if (type %in% c("p","P")) {
       colF$percent = TRUE
       colF$digits = 3
   }
   
   if (type %in% c("d")) colF$date = TRUE
   if (type %in% c("m")) colF$datetime = TRUE
   do.call(reactable::colFormat, colF)
}
.setColName = function(column, old) {
   if (is.null(column$title)) return (titleCase(column$name))
   if (column$title == "u")   return(toUpper(column$name))
   if (column$title == "l")   return(toLower(column$name))
   if (column$title == "a")   return(column$name)
   titleCase(column$name)
}
.setVisibility = function(column) {
   if (!is.null(column$type)  && column$type  == "h") return(FALSE)
   if (!is.null(column$title) && column$title == "h") return(FALSE)
   TRUE
}

.setStyle = function(column, df) {
   func = function(value) {
      sty = list()   
      if (!is.null(column$type)) {
          if (column$type %in% c("d","h","m"))  sty$`text-align` = "right"     
      }
      if (!is.null(column$color) && !column$color %in% c("x", " " )) {
#         sc = .colorScheme(column$color)
                    sc = YATATools::schemeOrange()
         if (column$color %in% c("d"))  {
            sc = YATATools::schemeRedGreen()
            normalized = YATATools::normalizeInterval(value - Sys.Date(), 15)
         }
         if (column$color %in% c("o"))  {
            #sc = YATATools::schemeOrange()
            sc = YATATools::schemeVariation()
            #normalized = YATATools::normalizeValue(value, df[,column$idx])
            normalized = YATATools::normalizeVariation(value)
         }
         color = sc(normalized)       
...
.formatData = function(df, data) {
 
    res = lapply(1:ncol(df), function(idx) {
     #  for (idx in 1:ncol(df)) {
       fmt = .formatColumn(idx, data)
       name = .setColName   (data$columns[[idx]], colnames(df)[idx])
       show = .setVisibility(data$columns[[idx]])
       sty  = .setStyle     (data$columns[[idx]], df)
       do.call(colDef, list( format=fmt
                            ,name = name
                            ,show = show
                            ,sortable = FALSE
                            ,style=sty
...
.updTable2 = function(data, selection) {
   click = NULL
