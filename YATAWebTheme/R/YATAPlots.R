...
upd2Plot = function(plot, ui, info) {
...
yataPlot = function(info, df, title) {
   ...
   if(!missing(title)) info$title = title
   plot = yataPlotBase()
   if (info$plot == "Candlestick") plot = plot %>% yataPlotCandlestick(df, info)
   if (info$plot == "Line")        plot = plot %>% yataPlotLine(df, info)
   plot
}
...
# upd2PlotModebar = function(plot, info) {
#    id = info$ui
#    if (is.null(plot)) {
#        shinyjs::hide(id)
...
#    if (!is.null(buttons)) plot = plot %>% plotly::config(modeBarButtonsToAdd = buttons)
# 
