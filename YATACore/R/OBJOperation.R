OBJOperation = R6::R6Class("OBJ.OPERATION"
    ...
    ,public = list(
        initialize = function(Factory) {
            ...
            private$prtOper        = Factory$getTable(YATACodes$tables$operations)
            private$tblFlows       = Factory$getTable(YATACodes$tables$flows)
            private$objPos         = Factory$getObject(YATACodes$object$position)
        }
        ##############################
        # Operaciones
        ##############################
        ...
        ,bid   = function(...) { add(YATACodes$oper$bid,  ...) }
        ,ask   = function(...) { add(YATACodes$oper$ask,  ...) }
        ...
        ,accept  = function(price=0, amount=0, fee = 0, id=NULL) {
           ...
           addFlow(YATACodes$flow$output, ctc, imp, price)
        ...
        ,execute = function(gas = 0, id=NULL) {
            ...
            tryCatch({
               db$begin()
               prtOper$update(data)
               addFlow(YATACodes$flow$input, ctc, cant, current$price)
...
#            tblOperLog$table(idOper = current$id)
        ...
        ,getActive     = function()    { getOperations(status=YATACodes$status$executed)  }
        ...
        ,getOpen       = function()    { getOperations(status=YATACodes$status$executed
                                                      ,active=YATACodes$flag$active) }
        ...
        ,getHistory    = function()    {
            dfCounter = prtOper$getInactiveCounters(YATACodes$flag$inactive)
        ...
        ,getClosed    = function()    {
            dfCounter = prtOper$getInactiveCounters(YATACodes$flag$parent)
            if (nrow(dfCounter) == 0) return (NULL)
            if (is.null(dfReg)) private$dfReg = objPos$getRegularizations()
            dfj = left_join(dfCounter, dfReg, by=c("camera", "counter"))
            if (nrow(dfj) == 0) return (NULL)
    ...
    ,private = list(
       ...
       ,operXfer     = function() {
           ...
           data = list(
               ...
               ,price   = value
               ,parent  = idXfer
               ,active  = YATACodes$flag$inactive
               ,status  = YATACodes$status$executed
               ,type    = YATACodes$oper$xfer
           )
           prtOper$add(data)
           ...
           flow$idOper = idIn
           flow$type   = YATACodes$flow$xferIn
...
#           amount = ifelse(current$base == "$FIAT", current$amount, current$value)
           ...
           addFlow(YATACodes$flow$output,  current$base,    current$amount * -1, current$price)
           addFlow(YATACodes$flow$input,   current$counter, current$value,       current$price)

           # if (!is.null(current$idParent) && !is.na(current$idParent)) {
           #     select(idParent)
           #     current$flag = YATACodes$flags$parent
      ...
      ,getRegularizationPeriod = function (camera, currency, since) {
       ...
       ,calculateExpense = function(camera, currency, amount) {
           ...
           while ( pend > 0) {
               ...
               if (df[nrow, "type"] == YATACodes$oper$xfer) break # Regularizacion.
               if (df[nrow, "type"] != YATACodes$oper$sell && df[nrow,"type"] != YATACodes$oper$buy) {
                   next
               }

               if (df[nrow,"type"] == YATACodes$oper$sell) {
