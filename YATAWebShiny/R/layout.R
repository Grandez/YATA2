JGGLayout = R6::R6Class("JGG.LAYOUT"
    ,portable   = FALSE
    ,cloneable  = FALSE
    ,lock_class = FALSE
    ,public = list(
          initialize    = function(id, blocks=NULL, items=NULL, selected=NULL, widgets=NULL) {
              ...
              if (is.null(blocks)) {
                  ...
                  if (!is.null(blocks$widgets))  private$widgets  = blocks$widgets
              ...
              if (!is.null(widgets))  {
                  if (is.null(private$widgets)) private$widgets = widgets
                  else                          private$widgets = list.merge(private$widgets, widgets)
              }
          }
        ,config = function(ns) {
            if (!missing(ns)) {
                private$id = ns(id)
                private$divBase = gsub("-", "_", private$id)
            }
            makeCombos()
        }
        ,body = function(ns)   {
    ...
    ,makeDivData = function() {
        ...
        for (i in 1:4) {
             ...
             divObj = appendWidgets(idDiv, idUI, i, divObj)
             divData = tagAppendChild(divData, divObj)
        }
        divData
    }
    ,appendWidgets = function(idDiv, idUI, index, divObj) {
       divs = divObj
       for (i in 1:length(private$widgets)) {
            if (!is.null(private$widgets[[i]]$widget)) {
                dv = tags$div(id=paste0(idDiv, names(private$widgets)[i], "_", index))
                wdg = setNameSpace(private$widgets[[i]]$widget, index, idUI)
                dv = tagAppendChild(dv, eval(parse(text=wdg)))
                divs = tagAppendChild(divs, dv)
             }
       }
       divs
    }
    ,setNameSpace = function(widget, block, idUI) {
        grep = gregexpr("ns\\([a-zA-Z0-9_]+\\)", widget)
        if (length(grep) == 0) return (widget)
        wdg = widget
        grep = grep[[1]]
        for (i in 1:length(grep)) {
           from=substring(widget, grep[i], grep[i] + attr(grep,"match.length")[i] - 1)
           to = substring(from, 4, nchar(from) - 1)
           to = paste0("'", idUI, to, "_", block, "'")
           wdg = gsub(from, to, wdg, fixed=TRUE)
        }
        wdg
    }
  )
