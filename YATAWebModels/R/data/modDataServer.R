...
modDatServer <- function(id, full, pnlParent, parent=NULL) {
   ns = NS(id)
...
#        #YATAWEB$beg("Position Server")
       ...
       if (is.null(pnl)) pnl = YATAWEB$addPanel(PNLData$new(session))

        observeEvent(input$pnlData, {
           act = yataActiveNS(input$pnlData)
           module = paste0("modDat", titleCase(act),"Server")
           eval(parse(text=paste0(module, "(act, input$pnlData, pnl, parent=parent)")))
    })
