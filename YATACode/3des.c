...
encryption is:  ciphertext = EK3(DK2(EK1(plaintext)))
decryption is:  plaintext = DK1(EK2(DK3(ciphertext)))
i.e., decrypt with K3, encrypt with K2, then decrypt with K1.
Input files to the program:
input.txt – This file should contain the plain text (Max. limit of plain text is 64kb).
 
key.txt – It should contain three 64-bit key (One after another without spaces). Consider
