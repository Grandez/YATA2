modOperSellServer = function(id, full, pnl) { 
    ns = NS(id)
#    dfPos        = NULL
#    dfCamera     = NULL
#    fee          = 0
#    browser()
    moduleServer(id, function(input, output, session) {
        updCombo("cboCamera", choices=pnl$cboCameras())
#       validate = function() {
#          FALSE
#       }
...
#       }, ignoreInit = TRUE)      
      observeEvent(input$cboCamera, {
          ...
          updateSelectInput(session, "cboCounter",     choices=pnl$cboCurrency(input$cboCamera, TRUE))
