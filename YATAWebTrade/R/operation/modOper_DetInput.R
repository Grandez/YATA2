...
modOperDetailInput = function(id, title="") {
  ...
  main = tagList(
     fluidRow( style="display: flex; margin: auto;", htmlOutput(ns("icon")), h3(yuiLabelText(ns("title"))))
    ,fluidRow(column(4, style="display: flex;"
    ...
    ,fluidRow(actionButton(ns("boton"), "Botoncito"))
     ,fluidRow(id=ns("plot1"), yuiPlot(ns("plot10")))
     ,hidden(fluidRow(id=ns("plot1x"), column(6,yuiPlot(ns("plot11"))), column(6,yuiPlot(ns("plot12")))))
     ,fluidRow(id=ns("plot2"), yuiPlot(ns("plot20")))
