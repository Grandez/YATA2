...
modOperDetailInput = function(id, title="") {
  ...
  main = tagList(
     yuiRow( column(1)
              ,column(4, htmlOutput(ns("icon")), h3(yuiLabelText(ns("title"))))
              ,column(4, style="display: flex;"
               ,tags$table(class="yata_tbl_monitor"
           ,tags$th(colspan="2", "Entrada")
           ,tags$tr(
              tags$td(class="yata_cell_label", "Fecha")
             ,tags$td(class="yata_cell_data",  id=ns("_cost_delta"))
           )
           ,tags$tr(
              tags$td(class="yata_cell_label", "Cantidad")
             ,tags$td(class="yata_cell_data",  id=ns("_cost_delta"))
           )
           ,tags$tr(
              tags$td(class="yata_cell_label", "Precio")
             ,tags$td(class="yata_cell_data",  id=ns("_cost_delta"))
           )
           ,tags$tr(
              tags$td(class="yata_cell_label", "Precio")
    ...
    ,yuiRow(actionButton(ns("boton"), "Botoncito"))
     ,yuiRow(id=ns("plot1"), yuiPlot(ns("plot10")))
     ,hidden(yuiRow(id=ns("plot1x"), column(6,yuiPlot(ns("plot11"))), column(6,yuiPlot(ns("plot12")))))
     ,yuiRow(id=ns("plot2"), yuiPlot(ns("plot20")))
     ,hidden(yuiRow(id=ns("plot2x"), column(6,yuiPlot(ns("plot21"))), column(6,yuiPlot(ns("plot22")))))
...
#             )
#      )  
#     ,yuiRow(yuiColumn(2), yuiColumn(4, h2("Notas")))
#     ,yuiRow(yuiColumn(2), yuiColumn(7, yuiTextArea(ns("comment"), label=NULL, cols="200", rows="20")))
#     ,yuiRow(yuiColumn(2), yuiYesNo(id=ns("tpl"), "Procesar", "Cancelar"))
#   )
