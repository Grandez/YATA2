  ...
  main = tagList(
    ...
    ,fluidRow(
        ...
        ,guiColumn(3
            ,fluidRow(h2("Control"))
            ,fluidRow(tags$table(
                 tags$tr( tags$td(WEB$MSG$get("LBL.TARGET"))
                         ,tags$td(yuiSwitch(ns("swTarget"),onLbl="%", offLbl="Value"))
                         ,tags$td(guiNumericInput(ns("target"), NULL, value = 5)))
                ,tags$tr( tags$td(WEB$MSG$get("LBL.DEADLINE"))
                         ,tags$td(colspan=2, guiIntegerInput(ns("deadline"), NULL, value = 7)))
                ,tags$tr( tags$td(WEB$MSG$get("LBL.STOP"))
                         ,tags$td(yuiSwitch(ns("swStop"),onLbl="%", offLbl="Value"))
                         ,tags$td(guiNumericInput(ns("stop"), NULL, value = 3)))
                ,tags$tr( tags$td(WEB$MSG$get("LBL.LIMIT"))
                         ,tags$td(colspan=2, guiNumericInput(ns("limit"), NULL, value = 0)))
                ,tags$tr( tags$td(WEB$MSG$get("LBL.REVIEW"))
                         ,tags$td(colspan=2, guiIntegerInput(ns("alert"), NULL, value = 3)))
            ))
        )
        ,guiColumn(3
            , fluidRow(h2(WEB$MSG$get("LBL.SUMMARY")))
            ,fluidRow(tags$table(
                 tags$tr(tags$td(WEB$MSG$get("LBL.AVAILABLE")), tags$td(guiLabelNumber(ns("lblAvailable"))))
                ,tags$tr(tags$td(WEB$MSG$get("LBL.AMOUNT")),    tags$td(guiLabelNumber(ns("lblAmount"))))
                ,tags$tr(tags$td(WEB$MSG$get("LBL.FEE")),       tags$td(guiLabelNumber(ns("lblFee"))))
                ,tags$tr(tags$td(WEB$MSG$get("LBL.GAS")),       tags$td(guiLabelNumber(ns("lblGas"))))
