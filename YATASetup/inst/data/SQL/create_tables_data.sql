...
exchangeId exchangeName baseSymbol baseCurrencyId quoteSymbol quoteCurrencyId
DROP TABLE  IF EXISTS EXCHANGES_PAIR CASCADE;
CREATE TABLE EXCHANGES_PAIR  (
    ID_EXCH     INTEGER      NOT NULL
   ...
   ,PRIMARY KEY ( ID_EXCH, ID_BASE, ID_COUNTER )
   ,UNIQUE  KEY ( ID_EXCH, BASE,    COUNTER )
);
...
-- Tabla de cambios por dia
DROP TABLE  IF EXISTS FIATS_EXCHANGE CASCADE;
CREATE TABLE FIATS_EXCHANGE  (
    BASE        CHAR(3)
   ,COUNTER     CHAR(3)
   ,CLOSE       DATE
   ,EXCHANGE      DOUBLE DEFAULT 1.0       
   ,PRIMARY KEY ( BASE, COUNTER,  CLOSE DESC)
);
