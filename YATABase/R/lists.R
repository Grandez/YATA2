...
jgg_list_combine = function(lst1, lst2) {
   ...
   while (idx1 <= length(lst1) && idx2 <= length(lst2)) {
       ...
       if (names1[idx1] == names[idx2]) {
           lst = list.merge(lst1[[idx1]], lst1[[idx2]])
   ...
   if (idx1 < length(lst1)) res = list.merge(res, lst1[[idx1:length(lst1)]])
   if (idx2 < length(lst2)) res = list.merge(res, lst2[[idx2:length(lst2)]])
   res
}
