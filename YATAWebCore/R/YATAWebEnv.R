...
#     ,window  = list(width = 0, height = 0)
     ...
     ,initialize = function() {
         ...
         tryCatch({
            ...
            private$rest = self$factory$getObject("Backend")
    ...
    ,getCurrencyNames = function(...) {
        if (is.null(private$ctcNames)) private$ctcNames = private$rest$names()
        args = list(...)
        ctcs = unlist(args)
        df   = private$ctcNames
        if (!is.null(ctcs)) {
