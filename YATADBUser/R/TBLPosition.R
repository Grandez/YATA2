TBLPosition = R6::R6Class("YATA.TABLE.POSITION"
  ,inherit    = YATATable
  ,portable   = FALSE
  ,cloneable  = FALSE
  ,lock_class = TRUE
  ,public = list(
      initialize = function(name, db=NULL) {
         super$initialize(name, private$tblName, fields=private$fields,key=key, db=db)
      }
     ,getGlobalPosition = function() {
         ...
         stmt = paste( "SELECT  CURRENCY, SUM(BALANCE) AS BALANCE, SUM(AVAILABLE) AS AVAILABLE"
                      ...
                      ,",MAX(BUY_LAST)  AS BUY_LAST,  MAX(SELL_LAST) AS SELL_LAST"
